...
Запуск с добавлением SSH-ключа в ssh-agent позволяет вводить пароль от приватного ключа один раз. После выполнения команды `ansible-playbook` запускается команда по удалению ключа `ssh-agent -k`:
```bash
